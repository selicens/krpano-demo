<script setup>
  ...
  const loadXmlString = () => {
    if (krpano) {
      ...
      krpano.value.call("loadxml(" + escape(xmlstring) + ", null, MERGE, BLEND(0.5));");
    }
  }
  const loadPano = () => {
  ...
  const lookToRandomView = () => {
    if (krpano) {
      krpano.value.call("lookto(" + (Math.random() * 360.0 - 180.0) + "," + (Math.random() * 180.0 - 90.0) + "," + (80.0 + Math.random() * 100.0) + ")");
    }
  }
  /*获取当前视图*/
